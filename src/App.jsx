...
export default function App() {
  ...
  const [screen, setScreen] = useState("home"); // home | list | history
  ...
  const updateList = (id, newItems) => {
    setLists((prev) => prev.map((l) => l.id === id ? { ...l, items: newItems } : l));
  };
  ...
  const duplicateList = (list) => {
    const nl = {
      ...
      items: (list.items || []).map((i) => ({ ...i, id: uid(), done: false })),
    };
  ...
  const fab = {
    ...
    boxShadow: "0 8px 24px rgba(110,231,183,.35)",
    cursor: "pointer",
  };
  ...
  const saveItem = () => {
    ...
    if (editItemId) {
      updateList(activeList.id, items.map((i) => i.id === editItemId ? { ...newItem, done: i.done } : i));
    } else {
      updateList(activeList.id, [...items, newItem]);
    }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  return (
    <div style={{ background: T.bg, minHeight: "100dvh", color: T.text, maxWidth: 480, margin: "0 auto", position: "relative" }}>
      ...
      <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "16px 20px", position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", gap: 12 }}>
        {screen !== "home" && (
          <button onClick={() => setScreen("home")} style={{ background: "none", border: "none", color: T.muted, fontSize: 22, cursor: "pointer", padding: "0 4px" }}>←</button>
        )}
        <div style={{ flex: 1 }}>
        ...
        {/* botões de header */}
        {screen === "home" && (
          <button onClick={() => setScreen("history")} style={{ background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 10, padding: "6px 12px", color: T.muted, fontSize: 13, cursor: "pointer" }}>
      ...
      {/* Barra de progresso na tela de lista */}
      {screen === "list" && activeList && (() => {
        const items = activeList.items || [];
        const pct = items.length > 0 ? (items.filter((i) => i.done).length / items.length) * 100 : 0;
        return (
          <div style={{ height: 3, background: T.surface2 }}>
            <div style={{ height: "100%", width: pct + "%", background: `linear-gradient(90deg, ${T.accent2}, ${T.accent})`, transition: "width .4s" }} />
          </div>
        );
      })()}
      ...
      {screen === "home" && (
        <div style={{ padding: "20px 20px 120px" }}>
              ...
              <div key={list.id} style={{ ...card, cursor: "pointer" }} onClick={() => openList(list.id)}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {editListId === list.id ? (
                      <input
                        ...
                        onKeyDown={(e) => { if (e.key === "Enter") saveRename(list.id); if (e.key === "Escape") setEditListId(null); }}
                        onBlur={() => saveRename(list.id)}
                        autoFocus
                        style={{ ...inputStyle, marginBottom: 0, fontSize: 17, fontWeight: 700 }}
                      />
                    ) : (
                      <>
                  ...
                  <div style={{ display: "flex", gap: 2 }} onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => { setEditListId(list.id); setEditListName(list.name); }} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", padding: "4px 6px", color: T.muted }}>✏️</button>
                    <button onClick={() => archiveList(list.id)} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", padding: "4px 6px", color: T.danger }}>📦</button>
                  </div>
                </div>
                {total > 0 && (
          ...
          {showNewList && (
            <div style={card}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setShowNewList(false)} style={{ ...btnSecondary, width: "auto", flex: 1 }}>Cancelar</button>
          ...
          {/* FAB */}
          {!showNewList && (
            <button onClick={() => setShowNewList(true)} style={fab}>+ Nova lista</button>
          )}
        </div>
      )}
          ...
          return (
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: `1px solid ${T.border}`, opacity: item.done ? .4 : 1, transition: "opacity .3s" }}>
              <button
                ...
                style={{ width: 28, height: 28, flexShrink: 0, borderRadius: 8, border: `2px solid ${item.done ? cat.color : T.border}`, background: item.done ? cat.color : "transparent", display: "flex", alignItems: "center", justifyContent: "center", color: "#0f1117", fontSize: 16, cursor: "pointer", transition: "all .2s" }}
              >{item.done ? "✓" : ""}</button>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {!item.done && <button onClick={() => openEditItem(item)} style={{ background: "none", border: "none", fontSize: 16, cursor: "pointer", color: T.muted, padding: "4px 6px" }}>✏️</button>}
              <button onClick={() => deleteItem(activeList.id, item.id)} style={{ background: "none", border: "none", fontSize: 16, cursor: "pointer", color: T.danger, padding: "4px 6px" }}>🗑</button>
            </div>
          );
        };
          ...
          <div style={{ padding: "0 20px 120px" }}>
            ...
            {showAddItem && (
              <div style={{ ...card, marginTop: 20 }}>
                ...
                <input value={itemQty} onChange={(e) => setItemQty(e.target.value)} placeholder="Quantidade (opcional, ex: 2, 500g...)" style={inputStyle} />
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {CATEGORIES.map((c) => (
                      <button key={c.id} onClick={() => setItemCat(c.id)} style={{ padding: "5px 12px", borderRadius: 999, fontSize: 12, border: `1px solid ${itemCat === c.id ? c.color : T.border}`, background: itemCat === c.id ? c.color + "22" : T.surface2, color: itemCat === c.id ? c.color : T.muted, cursor: "pointer", transition: "all .15s" }}>
                        {c.label}
                      </button>
                    ))}
                  </div>
            ...
            {!showAddItem && <button onClick={openAddItem} style={{ ...fab, borderRadius: "50%", padding: 0, width: 60, height: 60, fontSize: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>}
          </div>
        );
      })()}
